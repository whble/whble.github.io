...
async function buildQuickTryUI() {
    ...
    var tomlFileURL = "https://git.diybyte.com:52088/whble/esp_applications/raw/branch/main/config.toml"; // defaulting to rainmaker for now.
        ...
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            if (requestedApp) {
                ...
                if (filtered_apps.length > 0) {
                    ...
                    alert(`没有发现应用 ${requestedApp}`);
                }
            }
            if(!isDefault) {
                $("#qtLabel").html("从下面列表中选择固件. <Br> 您现在正在从一个<b><u>外部资源</u> - "
                                            + tomlFileURL + "</b>连接中选择应用固件程序。");
            }
            try {
...
function postConnectControls() {
    if(chipDesc !== "default"){
        ...
        $("#programwrapper").tooltip().attr("data-bs-original-title","将烧录固件到您的设备中");
        $("#flashingBaudrateSelect").prop("disabled", true);
        $("#flashButton").prop("disabled", false);
        $("#flashWrapper").tooltip().attr('data-bs-original-title', "将下载和烧录固件到您的设备中");
        ...
        lblConnTo.innerHTML = "<b><span style='color:red'>无法连接到设备. 请检查设备没有在其他的应用中占用!</span></b>";
...
disconnectButton.onclick = async () => {
    ...
    $("#flashWrapper").tooltip().attr('data-bs-original-title', "点击顶部连接按钮");
    $("#programwrapper").tooltip().attr("data-bs-original-title","点击顶部连接按钮");
...
function validate_program_inputs() {
    ...
    for (let index = 1; index < rowCount; index ++) {
        ...
        if (Number.isNaN(offset))
            return "偏移字段 " + index + " 不是一个有效的地址!";
        // Repeated offset used
        else if (offsetArr.includes(offset))
            return "偏移字段 " + index + " 已经在使用了!";
        else
...
function buildAppLinks(){
    ...
    let defaultAppURLsHTML = "注意：您可以从应用商店下载手机应用程序，并与您的设备进行交互。扫描二维码访问相应的应用程序。";
    let appURLsHTML = "";
    let setupPayloadInfo = "要设置设备，请使用支持的手机应用程序扫描位于最右侧的QRCode。";
...
function cleanUpOldFlashHistory() {
    ...
    $("#iosAppLogo").html("");
    $("#iosAppLogoQS").html("");
    $("#progressMsgQS").html("<i>需要等待一段时间，请在控制台查看进度！</i>");
...
flashButton.onclick = async () => {
    if(chipSetsRadioGroup.querySelectorAll("input[type=radio]:checked").length!== 0 &&
       ...
       developKitsRadioGroup.querySelectorAll("input[type=radio]:checked").length!== 0)){
        ...
        let alertChipsetSelectMsg = `<b><span style="color:red">无法烧录设备. 请检查烧录芯片是否已经选择</span></b>`;
